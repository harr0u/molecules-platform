...
case class PeriodicLennardJonesFrameLog[
  V <: AlgebraicVector[V],
  Fig <: GeometricFigure,
  Context[_] : Monad,
  T[_]: Traverse
...
)(implicit pc: PairwisePotentialCalculator[V]) extends FrameLog[V, Context, T, PeriodicLennardJonesFrameLog[V, Fig, Context, T]]
  with CenterOfMassReferentialFrame[V, Context, T, PeriodicLennardJonesFrameLog[V, Fig, Context, T]]
  with LeapFrogIteration[V, Context, T, PeriodicLennardJonesFrameLog[V, Fig, Context, T]]
  with SpaceLimitConditions[V, Fig, Context, T, PeriodicLennardJonesFrameLog[V, Fig, Context, T]] {

  override val potentialCalculator: PairwisePotentialCalculator[V] = pc

  override def updateWithParticlesState(particles: ParticlesState[V, Context, T]): Context[PeriodicLennardJonesFrameLog[V, Fig, Context, T]] = {
    Context.pure(this.copy())
  }
}
